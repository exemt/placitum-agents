 ...
 *
 * Канал cmd агент Redis собирает сам, из дельты счётчиков INFO между опросами
 * (redisinfo.Rate): отдельных команд он не видит, и кольца для него нет.
 * Кольцо ниже — ради канала log: собственный журнал агента уезжает в waf.log
 * пачками, и потери доставки видны в кадре так же, как у агента ноды. Код --
 * копия s3/agent/internal/flow: одна форма кадра у всех производителей io.
 */
...
// Flow — темп канала. In/Out нулём, если канал байты не считает: omitempty
// стирает поле, а не выдаёт "0", неотличимое от того, что операции реально
// ничего не передали.
type Flow struct {
...
// Counter — кольцо секундных корзин на Window секунд с гистограммой задержек.
type Counter struct {
	mu    sync.Mutex
	ops   [Window]uint64
	in    [Window]uint64
...
// AddN — n операций одной пачки, померенных одним общим временем.
